...
$(document).ready(function(){
  ...
  function(data){
    ...
    for(var i = 0 ; i < savedTodo.length; i++){
      $(".listBox").append("<li>"+savedTodo[i]["todo"]+"<button class='todoRemove' type='button'>삭제</button></li>")
    }
    $(".todoRemove").click(function(){
      var temp = $(this);
      var text = $(this).parent().html();
      var afterText = text.split('<');
      $.post("php/RemoveTodoList.php",
      {
        userID: $("#userID").text(),
        todo: afterText[0]
      },
...
//일기 작성 버튼 클릭시 일기 작성 페이지가 나타남
  ...
  $("#listClick").click(function(){
    var todo = $("#ToDo").val();
    $("#ToDo").val("");
    $(".listBox").append("<li>"+todo+"<button class='todoRemove' type='button'>삭제</button></li>")
...
function feelingSearch(year, month, feeling){
  ...
  function(data){
    ...
    for(var i = 0 ; i < day_array.length; i++){
      ...
      if(newCount > 0){
        $(".tableBox").append("<table><tr><th>"+today +"</th></tr><tr><td>'"+content+"'외 " +newCount +"개의 일기</td></tr></table>");
      }
      else{
        $(".tableBox").append("<table><tr><th>"+today+"</th></tr><tr><td>"+content+"</td></tr></table>");
...
function wheatherSearch(year,month,wheather){
  ...
  function(data){
    ...
    $("table").click(function(){
      ...
      $.post("php/wheatherReadDiary.php",
      {
        userID: $("#userID").text(),
        today: today,
        wheather: newWheather
